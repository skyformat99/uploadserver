...
type DataStore interface {
	...
	WriteChunk(id, pid string, offset int64, src io.Reader) (int64, error)
	...
	// 404 Not Found.
	GetInfo(id, pid string) (FileInfo, error)
	//external by caoge for return addrs what upload addrs can by http access
	GeturlAddr(objectName, pid string, offst int64) (string, error)
}
...
type FinisherDataStore interface {
	...
	FinishUpload(id, pid string) error
}
...
type GetReaderDataStore interface {
	...
	// be returned.
	GetReader(id, pid string) (io.Reader, error)
}
...
type ConcaterDataStore interface {
	...
	ConcatUploads(destination, pid string, partialUploads []string) error
}
...
type LengthDeferrerDataStore interface {
	DeclareLength(id, pid string, length int64) error
}
