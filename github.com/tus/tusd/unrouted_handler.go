...
var (
	...
	ErrInvalidPid                       = NewHTTPError(errors.New("missing or invalid pid header"), http.StatusBadRequest)
...
func (handler *UnroutedHandler) Middleware(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		handler.log(logger.TAIHETagRequestIn, "RequestIncoming", "method", r.Method, "path", r.URL.Path)
		et := elapsed.New()
		et.Start()
		...
		// if origin := r.Header.Get("Origin"); origin != "" {
			// logger.Info("reset origin:", origin)
		...
		// }
			...
			if r.Method == "OPTIONS" {
				...
				header.Add("Access-Control-Allow-Headers", " Origin, X-Requested-With, Content-Type, Upload-Length, Upload-Offset, Tus-Resumable, Upload-Metadata, Upload-Defer-Length, Upload-Concat, pid")
				header.Set("Access-Control-Max-Age", "86400")
				header.Set("Tus-Extension", "checksum")
				header.Set("Tus-Checksum-Algorithm", "md5")
			} else {
				// Actual request
		...
		defer func(et *elapsed.ElapTime){
			et.Stop()
			logger.Info("Request proc_time:", et.Elapsed())
		}(et)
		h.ServeHTTP(w, r)
	})
}
...
func (handler *UnroutedHandler) PostFile(w http.ResponseWriter, r *http.Request) {
	...
	if _, ok := meta["pid"]; !ok {
		...
		if !ok {
			...
			handler.sendError(w, r, ErrInvalidPid)
			return
		}
		pid = pidArr[0]
		meta["pid"] = pid
	...
	if isFinal {
		...
		size, err = handler.sizeOfUploads(partialUploads, pid)
		if err != nil {
			handler.sendError(w, r, err)
			return
		}
	...
	handler.log(logger.TAIHETagUploadCreated, "UploadCreated", "id", id, "size", i64toa(size), "url", url)
	...
	if isFinal {
		if err := handler.composer.Concater.ConcatUploads(id, pid, partialUploads); err != nil {
			handler.sendError(w, r, err)
			return
		}
		info.Offset = size
...
func (handler *UnroutedHandler) HeadFile(w http.ResponseWriter, r *http.Request) {
	pid := r.Header.Get("pid")
	if pid == "" {
		logger.Warn("not found pid delete failed")
		handler.sendError(w, r, ErrInvalidPid)
		return
	}
	...
	info, err := handler.composer.Core.GetInfo(id, pid)
	if err != nil {
		handler.sendError(w, r, err)
		return
	}
	...
	if info.Offset == info.Size {
		if _, ok := info.MetaData["fileurl"]; !ok {
			logger.Warn("something error i cannot find fileurl but the file upload had completed")
		} else {
			url := info.MetaData["fileurl"]
			logger.Info("file had existed and completed return fileurl directly fileurl:", url)
			//因为是HEAD请求,所以必须放在HEAD中
			w.Header().Set("Upload-Metadata", url)
			handler.sendResp(w, r, http.StatusOK)
			return
		}
	}
...
func (handler *UnroutedHandler) PatchFile(w http.ResponseWriter, r *http.Request) {
	...
	// fmt.Println("offset:%d\tinfo.Offset:%d\n", offset, info.Offset)
	logger.Info("offset:", offset, "\tinfo.Offset:", info.Offset)
	...
	if r.Header.Get("Upload-Length") != "" {
		...
		if err := handler.composer.LengthDeferrer.DeclareLength(id, info.MetaData["pid"],  uploadLength); err != nil {
			handler.sendError(w, r, err)
			return
		}
	...
	url, errUrl := handler.writeChunkV2(id, info, w, r)
	if errUrl != nil {
		err = errUrl
		handler.sendError(w, r, err)
		return
	...
	if url != "" {
		urlByte := []byte(url)
		handler.sendRespV2(w, r, urlByte, http.StatusOK)//client端确认后进行statusOK发送
		return
	}
	handler.sendResp(w, r, http.StatusNoContent)
	return
...
func (handler *UnroutedHandler) writeChunkV2(id string, info FileInfo, w http.ResponseWriter, r *http.Request) (url string, err error) {
	...
	handler.log(logger.TAIHETagChunkWriteStart, "ChunkWriteStart", "id", id, "maxSize", i64toa(maxSize), "offset", i64toa(offset))
	...
	if r.Body != nil {
		...
		//cheksum
		uploadChecksum := r.Header.Get("Upload-Checksum")
		if uploadChecksum != "" {
			logger.Info("uploadChecksum need info:", uploadChecksum)
			err = checksum(uploadChecksum, reader, length)
			if err != nil {
				logger.Warn("checksum failed err:", err)
				return
			}
		}

		bytesWritten, err = handler.composer.Core.WriteChunk(id, info.MetaData["pid"], offset, reader)
		if err != nil {
			return
		}
	}

	handler.log(logger.TAIHETagChunkWriteComplete, "ChunkWriteComplete", "id", id, "bytesWritten", i64toa(bytesWritten))
	...
	url, err = handler.finishUploadIfCompleteV2(info)
	return
}
...
func (handler *UnroutedHandler) finishUploadIfComplete(info FileInfo) error {
	...
	if !info.SizeIsDeferred && info.Offset == info.Size {
		...
		if handler.composer.UsesFinisher {
			if err := handler.composer.Finisher.FinishUpload(info.ID, info.MetaData["pid"]); err != nil {
				return err
			}
		}
...
// matches upload size) and if so, it will call the data store's FinishUpload
// function and send the necessary message on the CompleteUpload channel.
func (handler *UnroutedHandler) finishUploadIfCompleteV2(info FileInfo) (url string, err error) {
	// If the upload is completed, ...
	if !info.SizeIsDeferred && info.Offset == info.Size {
		// ... allow custom mechanism to finish and cleanup the upload
		if handler.composer.UsesFinisher {
		...
		go handler.Metrics.incUploadsFinished()
		url, err = handler.composer.Core.GeturlAddr(info.ID, info.MetaData["pid"], info.Offset)
		// fmt.Println(url)
		logger.Info("fileurl:", url)
		// /**
		//  * add by caoge bad use
		//  * 调用bos存储服务根据文件ID获取文件本地路径然后上传BOS
		//  * 后期优化为直接存储到BOS
		//  */
		//  bosAddr, errBos := bos.UploadBos(info.ID)
		//  if errBos != nil {
		//      err = errBos
		//  }
		//  url = bosAddr
...
func (handler *UnroutedHandler) GetFile(w http.ResponseWriter, r *http.Request) {
	...
	if pid == "" {
		logger.Warn("not found pid delete failed")
		handler.sendError(w, r, errors.New("pid invalid"))
		return
	}
	...
	if _, ok := info.MetaData["fileurl"]; ok {
		logger.Info(logger.TAIHETag302, "fileurl:", info.MetaData["fileurl"])
		//302从定向到file url
		w.Header().Set("Location", info.MetaData["fileurl"])
		handler.sendResp(w, r, http.StatusFound)//302
	} else {
		src, err := handler.composer.GetReader.GetReader(id, pid)
		if err != nil {
			handler.sendError(w, r, err)
			return
		}
...
func (handler *UnroutedHandler) DelFile(w http.ResponseWriter, r *http.Request) {
	...
	err = handler.composer.Terminater.Terminate(id, pid)
	if err != nil {
		handler.sendError(w, r, err)
		return
	}
...
func (handler *UnroutedHandler) sendError(w http.ResponseWriter, r *http.Request, err error) {
	...
	handler.log(logger.TAIHETagRequestOut, "ResponseOutgoing", "status", strconv.Itoa(statusErr.StatusCode()), "method", r.Method, "path", r.URL.Path, "error", err.Error())
...
func (handler *UnroutedHandler) sendResp(w http.ResponseWriter, r *http.Request, status int) {
	...
	handler.log(logger.TAIHETagRequestOut, "ResponseOutgoing", "status", strconv.Itoa(status), "method", r.Method, "path", r.URL.Path)
}

// sendResp writes the header to w with the specified status code.
func (handler *UnroutedHandler) sendRespV2(w http.ResponseWriter, r *http.Request, urlByte []byte, status int) {
	// Set headers before sending responses
	w.Header().Set("Content-Length", strconv.FormatInt(int64(len(urlByte)), 10))
...
// of a final resource.
func (handler *UnroutedHandler) sizeOfUploads(ids []string, pid string) (size int64, err error) {
	for _, id := range ids {
		info, err := handler.composer.Core.GetInfo(id, pid)
		if err != nil {
			return size, err
...
func checksum(uploadChecksum string, data io.Reader, size int64) (error){
	info := strings.Split(uploadChecksum, " ")
	algorithmName := info[0]
	algorithmValue := info[1]
	if algorithmName != "md5" {
		logger.Warn("sorry md5 supporset only")
		return errors.New("unkonw algorigthm")
	}
	logger.Info("algorithmName:", algorithmName, "\talgorithmValue:", algorithmValue)
	contentMD5, err := calculateContentMD5(data, size)
	if err != nil {
		logger.Warn("calculateContentMD5 failed:", err)
		return err
	}

	if contentMD5 != algorithmValue {
		logger.Warn("cheksum failed: ", algorithmValue, " != ", contentMD5)
		return errors.New("transfered something error please try agin")
	}
	return nil
}
...
func calculateContentMD5(data io.Reader, size int64) (string, error) {
	logger.Info("calculateContentMD5 size:", size)
	hasher := md5.New()
